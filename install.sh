...
if test -f "/etc/lsb-release" && grep DISTRIB_ID /etc/lsb-release >/dev/null && ! grep wrlinux /etc/lsb-release >/dev/null; then
  ...
  if test "$platform" = "xenserver"; then
    ...
    platform="xenserver"
  elif test "x$platform" = "xalibaba cloud linux"; then
    . /etc/os-release
    # PLATFORM_ID like platform:al8
    platform_version=`echo $PLATFORM_ID | sed 's/[^0-9]*\([0-9]\+\).*/\1/'`
    platform="el"
  else
    # FIXME: use "redhat"
    platform="el"
  ...
  case $platform in
    amazon*) # sh compat method of checking for a substring
      . /etc/os-release
